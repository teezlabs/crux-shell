import QtQuick
import Quickshell.Services.UPower
import qs.Commons
import qs.Modules.Bar.Extras

// Power profile quick toggle: cycles power-saver -> balanced ->
// performance. Unavailable where no power-profiles-daemon is running.
CcCircleToggle {
  id: tile
  signal expandRequested(string which)

  available: PowerProfileService.available
  active: PowerProfileService.available && PowerProfileService.profile !== PowerProfile.Balanced
  onTapped: PowerProfileService.cycle()

  Canvas {
    anchors.centerIn: parent
    width: 16
    height: 16
    readonly property int profile: PowerProfileService.profile
    readonly property color drawColor: tile.active ? Color.primaryContainerText : Color.surfaceText
    onProfileChanged: requestPaint()
    onDrawColorChanged: requestPaint()
    onPaint: {
      const ctx = getContext("2d");
      ctx.reset();
      ctx.strokeStyle = drawColor;
      ctx.fillStyle = drawColor;
      ctx.lineWidth = 1.4;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      const cx = width / 2;
      const cy = height / 2;
      if (profile === PowerProfile.Performance) {
        ctx.beginPath();
        ctx.moveTo(cx + 2.5, 1);
        ctx.lineTo(cx - 3.5, cy + 0.5);
        ctx.lineTo(cx - 0.5, cy + 0.5);
        ctx.lineTo(cx - 2.5, height - 1);
        ctx.lineTo(cx + 3.5, cy - 0.5);
        ctx.lineTo(cx + 0.5, cy - 0.5);
        ctx.closePath();
        ctx.fill();
      } else if (profile === PowerProfile.PowerSaver) {
        ctx.beginPath();
        ctx.moveTo(cx - 5, cy + 5);
        ctx.quadraticCurveTo(cx - 5, cy - 5, cx + 5, cy - 5);
        ctx.quadraticCurveTo(cx + 5, cy + 5, cx - 5, cy + 5);
        ctx.closePath();
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx - 3, cy + 3);
        ctx.lineTo(cx + 3, cy - 3);
        ctx.stroke();
      } else {
        ctx.beginPath();
        ctx.arc(cx, cy + 3, 5.5, Math.PI, Math.PI * 2);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(cx, cy + 3);
        ctx.lineTo(cx + 3, cy - 2);
        ctx.stroke();
      }
    }
  }
}
